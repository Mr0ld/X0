...
try:
    pydroid3 = "/data/data/ru.iiec.pydroid3/files"
    pyc = "pycdc2"
    pyc_old = os.path.join(pydroid3, "pycdc")

    if not os.path.isfile(os.path.join(pydroid3, pyc)):      
        shutil.move(pyc, pydroid3)
        os.chmod(os.path.join(pydroid3, pyc), 0o755)
        print("تم تفعيل pycdc بنجاح")

    elif os.path.isfile(pyc_old):
        os.remove(pyc_old)
        print("تم حذف pycdc القديم")
...
try:
  ...
  from cfonts import render
    ...
    os.system('pip install rich requests autopep8 python-cfonts')
...
flag = render("DECODE", font='block', align='center', colors=['red'], background='white')
print(flag)
...
if AHMED:
    ...
    if h in source:
        source = source.replace(h, "(url=c, headers=head1, data=data1)")
    ...
    h = "headers_, (headers)"
    if h in source:
        source = source.replace(h, 'headers=headers_')
		...
		if xe3 in file:
			...
			if n >0:
				...
				try:
					...
					print('error PytonsAhmed 1')
			else:
				print('error PytonsAhmed 2')
...
layrs=0
na=input("\033[34m\033[1m\033[4mEnter Your File Name To Decode It : \033[0m")
d=1
Ahmed(na,d)
try:
 os.remove("3737373737373")
 os.remove("decoded.py")
 os.remove('dec_dbdbdbdhd.py')
except:
 pass